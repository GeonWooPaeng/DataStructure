...
#소인수 분해 
#x를 점차 줄여가면 서 제일 작은 소인수 부터 나눠 준다ㅏ.
def prime(x):
    p, fac = 2, [] #소수의 시작, 소수를 저장할 곳
    while p ** 2 <= x: 
        if x % p == 0:
            x //= p 
            fac.append(p)
...
#어떤 수를 기준으로 해당 수의 배수를 다 지워가며 소수를 찾는 방법
def era_prime(x):
	a, p = [0 for _ in range(x+1)], [] #a: 소수인 경우 0나타낸다. p: 소수값 저장 list
	for i in range(2, x):
		#소수인 경우
		if a[i] == 0:
			p.append(i)
